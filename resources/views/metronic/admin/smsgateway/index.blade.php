        ...
        <div class="col-md-12">
            <div class="portlet box blue-hoki">
                <div class="portlet-title">
                    ...
                    <a href="{{route('sms_gateway_create')}}" title="New SMS Gateway" class="panel-title-btn btn btn-icon waves-effect waves-light btn-success btn-back pull-right" > <i class="ion-plus-circled"></i> </a>
                </div>
                <div class="portlet-body">
                    <div class="">
                        @include('layouts.alert')
                        <table class="table table-striped table-hover"  id="sms_gateway">
                        <thead>
                            <tr>
                                <th>
                                    #
                        ...
                        <tbody>
                            <?php $sl = 0; ?>
                            @foreach($smsGatewayList as $smsGateway)
                            <tr>
                                <td>{{ ++$sl }}</td>
                                <td>{{ $smsGateway['name'] }}</td>
                                <td>{{ $smsGateway['url'] }}</td>
                                <td>{{ $smsGateway['username'] }}</td>
                                <td>
                                    <span class="label label-sm label-{{ $smsGateway['active_status'] == 1 ? 'success' : 'danger' }} ">
                                    {{ $smsGateway['active_status'] == 1 ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td>
                                    <a href="{{route('sms_gateway_edit',$smsGateway['id'])}}" class="btn btn-icon-only purple" title="Edit">
